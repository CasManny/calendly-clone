...
CREATE TABLE IF NOT EXISTS "scheduleTable" (
	...
	"createdAt" timestamp DEFAULT now() NOT NULL,
	"updatedAt" timestamp DEFAULT now() NOT NULL,
	CONSTRAINT "scheduleTable_clerkUserId_unique" UNIQUE("clerkUserId")
);
--> statement-breakpoint
DO $$ BEGIN
 ALTER TABLE "scheduleAvailabilities" ADD CONSTRAINT "scheduleAvailabilities_scheduleId_scheduleTable_id_fk" FOREIGN KEY ("scheduleId") REFERENCES "public"."scheduleTable"("id") ON DELETE cascade ON UPDATE no action;
EXCEPTION
 WHEN duplicate_object THEN null;
END $$;
--> statement-breakpoint
